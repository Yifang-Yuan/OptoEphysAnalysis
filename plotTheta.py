...
def align_theta_by_trough (lfps,zscores,start_idx,end_idx,midpoint,Fs=10000):
    ...
    aligned_theta_band_lfps = np.zeros_like(lfps)
...
def plot_theta_aligned_by_trough (save_path,LFP_channel,recordingName,theta_triggered_lfps,theta_triggered_zscores,Fs=10000):
    ...
    '''Align by peak'''
    aligned_theta_band_lfps,aligned_lfps,aligned_zscores = align_theta_by_trough (theta_triggered_lfps,
                                                                         theta_triggered_zscores,start_idx,end_idx,midpoint,Fs)
    
    '''PLOT HEATMAP after aligning by peak'''
    fig=plot_theta_heatmap(aligned_theta_band_lfps,aligned_lfps,aligned_zscores,Fs)
    fig_path = os.path.join(save_path, 'Theta_aligned_heatmap_1s.png')
    fig.savefig(fig_path, transparent=True)
    
    fig=plot_theta_heatmap(aligned_theta_band_lfps[:,start_idx:end_idx],
    ...
    with open(save_file_path, "wb") as file:
        pickle.dump(aligned_theta_band_lfps, file)
...
def run_theta_plot_selectpeak (dpath,LFP_channel,recordingName,savename,theta_low_thres=0.5):
    ...
    '''theta DETECTION
    For a rigid threshold to get larger amplitude theta events: Low_thres=3, for more theta events, Low_thres=1'''
    Recording1.pynappleThetaAnalysis (lfp_channel=LFP_channel,
                                                ep_start=0,ep_end=10,
                                                Low_thres=theta_low_thres,High_thres=8,
                                                plot_segment=False,plot_theta_ep=False)
