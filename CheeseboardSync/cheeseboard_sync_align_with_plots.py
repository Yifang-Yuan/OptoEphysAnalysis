...
#!/usr/bin/env python3
"""
Cheeseboard: align Behaviour ↔ Ephys ↔ ATLAS Photometry + QC plots

Adds plots:
1) Full aligned window (~SPAD duration):
   - LFP1..LFP4 vs time (ephys seconds)
   - Z-score photometry vs time
2) Searching epoch (from entry to approach):
   - LFP1..LFP4 vs time (subset)
   - Z-score vs time (subset). If no approach, prints 'not found' and uses entry→end.
3) Animal speed (px/s) from entry to end of aligned window.

Figures are SHOWN (not saved) by default.
"""
...
def load_phot_csvs(sync_folder: str) -> Dict[str, np.ndarray]:
    def read_one(name: str) -> Optional[np.ndarray]:
        p = os.path.join(sync_folder, name)
        if not os.path.isfile(p): return None
        arr = pd.read_csv(p, header=0).iloc[:,0].to_numpy(float)
        return arr
    return {
        "green": read_one("Green_traceAll.csv"),
        "red":   read_one("Red_traceAll.csv"),
        "z":     read_one("Zscore_traceAll.csv"),
    }

# -------------------- Plot helpers --------------------

def plot_lfps(t: np.ndarray, lfps: Dict[str, np.ndarray], title: str):
    plt.figure(figsize=(12, 6))
    for k in ["LFP_1","LFP_2","LFP_3","LFP_4"]:
        if k in lfps:
            plt.plot(t, lfps[k], label=k, linewidth=1)
...
def plot_zscore(t: np.ndarray, z: Optional[np.ndarray], title: str):
    if z is None: 
        print("[warn] No Z-score photometry found; skipping plot.")
        return
...
def plot_speed(t_beh: np.ndarray, head_xy: Tuple[np.ndarray,np.ndarray],
               entry_time: Optional[float], end_time: float, title: str = "Speed (px/s)"):
    hx, hy = head_xy
    # Build mask from entry→end and finite coords
    valid = np.isfinite(hx) & np.isfinite(hy) & np.isfinite(t_beh)
    if entry_time is not None:
        valid &= (t_beh >= entry_time)
    if end_time is not None:
        valid &= (t_beh <= end_time)
    x = hx[valid]; y = hy[valid]; t = t_beh[valid]
    if len(t) < 2:
        print("[warn] Not enough behaviour points to compute speed.")
        return
    dt = np.diff(t)
    dx = np.diff(x); dy = np.diff(y)
    # guard against dt=0
    dt[dt == 0] = np.nan
    speed = np.hypot(dx, dy) / dt  # px/s
    tc = (t[:-1] + t[1:]) * 0.5
    plt.figure(figsize=(12,4))
    plt.plot(tc, speed, linewidth=1)
    plt.xlabel("Time (s)"); plt.ylabel("Speed (px/s)")
    plt.title(title)
    plt.tight_layout()
    plt.show()
...
def align_single_trial(sync_folder: str, beh_folder: str, trial_idx_one_based: int,
                       ...
                       save_pickle: bool = True, make_plots: bool = True) -> Dict:
    """
    Align one SyncRecordingN with Behaviour index k=N-1. Also generates QC plots.
    """
    ...
    # Ensure Cam_sync and DLC frames match length
    dlc = read_dlc_filtered(dlc_csv); scorer = get_top_scorer(dlc)
    n_cam = len(cam); n_dlc = len(dlc); n = min(n_cam, n_dlc)
    if n_cam != n_dlc:
        print(f"[warn] Frame count mismatch: Cam_sync={n_cam}, DLC={n_dlc}. Trimming to {n}.")
    cam = cam.iloc[:n].reset_index(drop=True)
    dlc = dlc.iloc[:n].reset_index(drop=True)

    head_xy = xy_of(dlc, "head", scorer); bottom_xy = xy_of(dlc, "bottom", scorer)

    cam_ts_s = (cam["Timestamp"] - cam["Timestamp"].iloc[0]).dt.total_seconds().to_numpy()
    ...
    # Assert alignment: NaN/NaN frame must map to cam_mask start time
    if not (abs(t_beh_ephys[cam_start_nan_idx] - t_cam_start_ephys) < 1e-6):
        raise AssertionError("Behaviour alignment error: NaN/NaN frame does not map to cam_mask start.")
    ...
    if make_plots:
        # 1) Full window LFPs
        plot_lfps(ephys_win["t"], ephys_win, title="LFP (full aligned window)")

        # 2) Full window Z-score
        plot_zscore(phot_pack["t"], phot_pack.get("z"), title="Photometry Z-score (full aligned window)")

        # 3) Searching epoch (entry -> approach or entry -> end if approach missing)
        if entry_time_ephys is not None:
            t0 = entry_time_ephys
            if approach_time_ephys is not None and approach_time_ephys > entry_time_ephys:
                t1 = approach_time_ephys
            else:
                print("[info] Approach event not found or before entry; using entry→end window for 'searching'.")
                t1 = t_spad_end_ephys

            # LFP subset
            mask_search = (ephys_win["t"] >= t0) & (ephys_win["t"] <= t1)
            lfps_sub = {k: v[mask_search] for k, v in ephys_win.items() if k != "t"}
            plot_lfps(ephys_win["t"][mask_search], lfps_sub, title="LFP (searching epoch)")

            # Z-score subset
            mask_z = (phot_pack["t"] >= t0) & (phot_pack["t"] <= t1)
            z_sub = phot_pack.get("z")
            plot_zscore(phot_pack["t"][mask_z], z_sub[mask_z] if z_sub is not None else None,
                        title="Photometry Z-score (searching epoch)")
        else:
            print("[warn] Entry event not found; skipping 'searching' plots.")

        # 4) Speed from entry → end
        end_time = t_spad_end_ephys
        plot_speed(beh["t"], beh["head"], entry_time_ephys, end_time, title="Head speed (px/s) from entry to end")
...
if __name__ == "__main__":
    ...
    # }
    # align_single_trial(sync_folder, beh_folder, trial_idx_one_based=1,
    #                    landmarks=landmarks, spad_fs=1682.92,
    #                    save_pickle=True, make_plots=True)
    pass
