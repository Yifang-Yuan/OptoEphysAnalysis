...
def single_trial_ripples(
    ...
    if neckX is not None:
        ...
        both_out = (~in_head) & (~in_neck)   # leave == both outside
    else:
        both_out = (~in_head)
    ...
    # optional z-scored photometry for plotting
    ...
    # ================= overview strictly inside [approach, leave] =================
    ...
    if ZSTD is not None:
        axZ.plot(ZSTD_rw.index.values, ZSTD_rw.values, color='#2ca02c', lw=0.8, label=f'{opt_label} (z)')
    ...
    # Detect on the reward window, then keep peaks/epochs inside immobility
    ...
    for i in range(n):
        ...
        # require enough room around the peak inside the full LFP
        if (t_peak - LFP.index.values[0]) < (half + pad) or (LFP.index.values[-1] - t_peak) < (half + pad):
            continue

        # big window for filtering, then crop to ±half
        ...
        # nSS in the same small window (use nSS from reward-window detection)
        nSS_win = nSS_rw.restrict(win_small)

        # photometry window (z-scored for plotting)
        Z_win = ZSTD.restrict(win_small) if ZSTD is not None else None

        # ---------- plotting ----------
        LBL_FS   = 18   # axis labels
        TICK_FS  = 14   # tick labels
        TITLE_FS = 18   # figure title
        ...
        # --- TOP: nSS + ripple-band (twin y) ---
        ax1.plot(nSS_win.index.values - t_peak, nSS_win.values,
                 color='0.3', lw=1.0, label='nSS (z)')
        ax1.axvline(0, color='r', lw=1.2)
        ax1.set_ylabel("Envelope (z)", fontsize=LBL_FS)
        
        ax1b = ax1.twinx()
        ax1b.plot(RB_win.index.values - t_peak, RB_win.values,
                  color='C0', lw=1.0, label='Ripple band (130–250 Hz)')
        ax1b.set_ylabel("Ripple band (a.u.)", color='C0', fontsize=LBL_FS)
        ax1b.tick_params(axis='y', labelcolor='C0', labelsize=TICK_FS)
        
        # --- MIDDLE: raw LFP ---
        ax2.plot(LFP_win.index.values - t_peak, LFP_win.values, color='k', lw=1.0)
        ax2.axvline(0, color='r', lw=1.2)
        ax2.set_ylabel("LFP", fontsize=LBL_FS)
        
        # --- BOTTOM: GEVI z (if present) ---
        if Z_win is not None:
            ax3.plot(Z_win.index.values - t_peak, Z_win.values, color='#2ca02c', lw=1.0)
            ax3.axvline(0, color='r', lw=1.2)
            ax3.set_ylabel("Zscore", fontsize=LBL_FS)
            ax3.set_xlabel("Time (s)", fontsize=LBL_FS)
        else:
            ax2.set_xlabel("Time (s)", fontsize=LBL_FS)
        
        # ---------- unified styling / frames / margins ----------
        def _flush_x(ax):
            ax.set_xlim(X0, X1)
            ax.margins(x=0, y=0)
            ax.set_xmargin(0)
        
        for a in ([ax1, ax1b, ax2] + ([ax3] if Z_win is not None else [])):
            _flush_x(a)
            a.tick_params(axis='y', labelsize=TICK_FS)
        
        # share x & hide top two x tick labels
        for a in [ax1, ax2]:
        ...
        # x-ticks: only 5 evenly spaced ticks
        for a in [ax2] + ([ax3] if Z_win is not None else []):
            a.set_xticks(np.linspace(X0, X1, 5))
            a.tick_params(axis='x', labelsize=TICK_FS)
        
        # figure title
        dur_ms = ((rip_ep.iloc[[i]]['end'] - rip_ep.iloc[[i]]['start']) * 1000)[0]
        fig.suptitle(f"Ripple {i-10} — (dur {dur_ms:.0f} ms)",
                     fontsize=TITLE_FS)
        
        # layout
        plt.subplots_adjust(left=0.12, right=0.88, top=0.90, bottom=0.12, hspace=0.02)
...
if __name__ == "__main__":
    sync_dir = r"G:\2025_ATLAS_SPAD\CB_Jedi2P\1910567\Success\Day2\SyncRecording3"
    landmarks = {
